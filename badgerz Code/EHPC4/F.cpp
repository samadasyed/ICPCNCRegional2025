...
int main()
{
    ...
    vector<vector<int>> crit2(r, vector<int>(c));
    ...
    for(int i = 0 ; i < r; ++i)
    {
        for(int j = 0 ; j < c; ++j)
        {
            // count number of prints within 3 dist
    ...
    vector<vector<int>> parent(r, vector<int>(c));
    int id = 0;
    for(int i = 0 ; i < r ; ++i)
    {
        for(int j = 0 ; j < c; ++j)
    ...
    while(!q.empty())
    {
        ...
        for(int dx = -2 ; dx <= 2; ++dx)
        {
            for(int dy = -2 ; dy <= 2; ++dy)
            {
                ...
                if(spid[cr+dx][cc+dy] ==1 && parent[cr+dx][cc+dy] != parent[cr][cc]) {
                    parent[cr+dx][cc+dy] = parent[cr][cc]; 
                    q.emplace(cr+dx, cc+dy);
                }
            }
